#ifndef __CAR_STRUCT_HPP
#define __CAR_STRUCT_HPP
...
std::string car_data_columns[] { "MANUFACTURER", "MODEL", "HORSEPOWER", "DOOR_COUNT" };
...
struct car_struct_t
{
    ...
    car_struct_t(std::vector<std::string> data_tokens, std::vector<std::string> data_header_tokens)
    {
        for(std::size_t i = 0; i < data_tokens.size(); i++)
        {
            std::cout << "car_struct_t: data_tokens: " << data_tokens[i] << '\n';
        }
        for(std::size_t i = 0; i < data_header_tokens.size(); i++)
        {
            std::cout << "car_struct_t: data_header_tokens: " << data_header_tokens[i] << '\n';
        }
    }

    car_struct_t(std::string _manufacturer, std::string _model, int _horsepower, int _door_count)
    {
    ...
    bool operator == (const car_struct_t& rhs_struct) const
    {
